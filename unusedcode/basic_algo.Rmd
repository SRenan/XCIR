...
$$
\begin{aligned}
P(X|Z) &= \prod_{j=1}^m P(Xi|Z_i = 1)P(Z_i = 1) + P(Xi|Z_i = 2)P(Z_i = 2)\\
L &= \prod_{j=1}^m \left[ \binom{n_j}{x_j} \theta_1^{x_j}(1-\theta_1)^{n_j-x_j} p_1 \right]^{z_i} + 
                 \left[ \binom{n_j}{x_j} \theta_2^{x_j}(1-\theta_2)^{n_j-x_j} p_2 \right]^{1-z_i}\\
logL &= \sum_{j=1}^m [z_i] \left[ \ln(p_1) + \ln \binom{n_j}{x_j} + x_j \ln(\theta_1) + (n_j-x_j) \ln(1-\theta_1) \right] +
                     [1-z_i] \left[ \ln(p_2) + \ln \binom{n_j}{x_j} + x_j \ln(\theta_2) + (n_j-x_j) \ln(1-\theta_2) \right]\\
\end{aligned}
$$

Now maximize Q: Derive Q with respect to each parameter theta1, theta2

$$
\begin{aligned}
\frac{\partial Q}{\partial \theta_1} &= \sum_{j=1}^m [z_i] \left[ \frac{x_j}{\theta_1} + \frac{n_j - x_j}{1-\theta_1} \right]\\
\theta_1^{(t+1)} &= \\
\end{aligned}
$$
...
while (abs(Q[k]-Q[k-1])>=1e-6){
  # E step
  # Likelihood of each datapoint in every mixture at current parameter values
